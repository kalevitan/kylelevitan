...
const Project: React.FC<ProjectProps> = ({name, image, description, features}) => {
  return (
      ...
      <div className="portfolio__details--wrapper">
        ...
        <ul className="portfolio__details">
          {features?.map((feature, index) => (
            <li className="flex items-center" key={index}><i className="lni lni-chevron-left rotate-180"></i>{feature}</li>
          ))}
        </ul>
      </div>
    </div>
